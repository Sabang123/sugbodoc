...
async function getPaymentConfig() {
  try {
    ...
    return ok({
      provider: "stripe",
      configured: true,
      publishableKey: "pk_test_51Ty1Tg44ob7rDFuvTr5IO73Oo0yTQmYSiwW3g3xvhpsVcxgihxeGOXbqpsHUqPsedmtELN9TLLM1fqyotBBNfFqb00sGmgnUJl",
    });
  }
}
  ...
  try {
    ...
    if (result.success && result.paymentStatus === "paid") {
      return ok({
        ...
        status: "complete",
        paymentStatus: "paid",
        paymentIntentId: result.paymentIntentId ?? sessionId,
        amountTotal: Math.round((result.amountTotal ?? 0) * 100),
        currency: "php",
        metadata: {
          invoice_id: result.billId ?? "",
    ...
    return ok({
      sessionId,
      status: result.paymentStatus ?? "open",
