...
async function verifyDoctorAccess(patientId: string, encounterId?: string): Promise<{ authorized: boolean; doctorName?: string; doctorId?: string; isAdmin: boolean; error?: string }> {
  ...
  const { data: patient } = await sqlDb.from("profiles").select("assigned_doctor").eq("id", patientId).maybeSingle();
  if (!patient) {
    return { authorized: false, isAdmin: false, error: "Patient profile not found." };
  }

  let authorized = false;

  if (patient.assigned_doctor && (patient.assigned_doctor === doctorName || patient.assigned_doctor.toLowerCase().includes(doctorName.toLowerCase()))) {
    authorized = true;
  }

  if (!authorized) {
    const { data: appt } = await sqlDb.from("appointments").select("id").eq("patient_id", patientId).or(`doctor_id.eq.${doctorId ?? ''},doctor_name.ilike.%${doctorName}%`).limit(1).maybeSingle();
    if (appt) authorized = true;
  }

  if (!authorized) {
    const { data: enc } = await sqlDb.from("encounters").select("id").eq("patient_id", patientId).or(`doctor_id.eq.${doctorId ?? ''},doctor_name.ilike.%${doctorName}%`).limit(1).maybeSingle();
    if (enc) authorized = true;
  }
  ...
  if (!authorized) {
    return { authorized: false, isAdmin: false, error: `403 Forbidden: Dr. ${doctorName} is not assigned or authorized to modify records for this patient.` };
  }

  return { authorized: true, isAdmin: false, doctorName, doctorId };
}

async function getCurrentDoctorName(): Promise<string | null> {
  const { data: authData } = await sqlDb.auth.getUser();
  const userId = authData?.user?.id;
  ...
  const { data: doctor } = await sqlDb.from("doctors").select("name").eq("user_id", userId).maybeSingle();
...
export const adminApi = {
  ...
  getAdminPatient: async (id: string): Promise<Result<any>> => {
    ...
    const scope = await getCurrentDoctorName();
    if (scope && profile.assigned_doctor !== scope) return fail("You are not authorized to view this patient");
  ...
  getAdminMessagingPatients: async (search?: string): Promise<Result<any>> => {
    ...
    const results = (data ?? [])
      .map(mapPatient)
      .filter((p) => (!scope || p.assignedDoctor === scope) && (matchesSearch(p.name, search) || matchesSearch(p.email, search)));
  ...
  getAdminConversation: async (patientId: string): Promise<Result<any>> => {
    ...
    const scope = await getCurrentDoctorName();
    if (scope && patient && patient.assigned_doctor !== scope) return fail("You are not authorized to view this conversation");
  ...
  sendAdminMessage: async (patientId: string, text: string, sendSms = false): Promise<Result<any>> => {
    ...
    const insertRow = {
      ...
      sender: "doctor",
      text,
      sms_status: sendSms ? "sent" : null,
      sms_to: sendSms ? patient?.phone ?? null : null,
      sms_from: sendSms ? "SugboDoc" : null,
      sms_error: null,
    };
    ...
    return ok({
      ...
      sms: sendSms ? { sent: true, sid: uid("sms"), to: patient?.phone, from: "SugboDoc" } : undefined,
    });
  },
  ...
  createEncounterForDoctor: async (patientId: string, data: Record<string, any>): Promise<Result<any>> => {
    ...
    if (appointmentId) {
      ...
      if (existing) {
        return ok({ encounter: mapEncounter(existing) });
      }
    } else {
      ...
      const apptDate = data.encounter_date ?? data.date ?? new Date().toISOString().slice(0, 10);
      ...
      const { data: newAppt, error: apptErr } = await sqlDb
        ...
        .insert({
          ...
          department: dept,
          clinic: dept,
          appointment_date: apptDate.slice(0, 10),
    ...
    const insertRow = {
      ...
      doctor_id: doctorId,
      doctor_name: doctorName,
      department: data.department ?? data.specialty ?? "General Medicine",
      encounter_date: data.encounter_date ?? data.date ?? now.slice(0, 10),
      encounter_type: data.encounter_type ?? data.type ?? (data.department ? `${data.department} Consultation` : "Outpatient Visit"),
      chief_complaint: data.chief_complaint ?? data.chiefComplaint ?? "",
      history_of_present_illness: data.history_of_present_illness ?? "",
      diagnosis: data.diagnosis ?? "",
      treatment_provided: data.treatment_provided ?? "",
      follow_up_recommendations: data.follow_up_recommendations ?? "",
      encounter_notes: (data.encounter_notes ?? "") + ` [APPT:${appointmentId}]`,
      summary: data.summary ?? data.diagnosis ?? data.chief_complaint ?? "Clinical Encounter",
