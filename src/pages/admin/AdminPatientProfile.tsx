  ...
  const handleSaveModal = async (formData: any) => {
    ...
    try {
      if (activeModal === 'new-encounter') {
        ...
        if (res.data?.encounter?.id) {
          setSelectedEncounterId(String(res.data.encounter.id));
        }
        refreshPatient();
    ...
    return (
      <div className="py-16 max-w-md mx-auto text-center space-y-4">
        ...
        <button
          ...
          className="mt-2 inline-flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-lg font-medium text-sm hover:bg-primary/90 transition-colors"
        >
            ...
            {allEncounters.length > 0 ? (
              <div className="space-y-1.5 max-h-60 overflow-y-auto pr-1">
                {allEncounters.map(encounter => (
                  <button key={encounter._id} onClick={() => setSelectedEncounterId(encounter._id)} className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors border ${selectedEncounterId === encounter._id ? 'bg-primary/10 border-primary/30 text-primary font-medium' : 'border-transparent hover:bg-muted text-foreground'}`}>
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="date" required value={formData.encounter_date ?? ''} onChange={e => handleChange('encounter_date', e.target.value)} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Department / Specialty *</label>
                  <input type="text" required value={formData.department ?? ''} onChange={e => handleChange('department', e.target.value)} placeholder="e.g. Cardiology, Internal Medicine" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
              </div>
              <div>
